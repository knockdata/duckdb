...
#
# The version is <upstream duckdb>-r.<revision>, so 1.5.5-r.1 is our first build of DuckDB
# v1.5.5. Bump the revision when only our build or JS changed; bump the base when DuckDB moved
# (build.sh reads the same field, so there is nothing else to edit).
#
# Retagging is the normal way to retry. scripts/publish.mjs skips any package already on npm, so
# rerunning a partially published release finishes it rather than failing on what succeeded last
# time — no revision is burned per attempt.
#
#   ./release.sh
set -euo pipefail
...
# Not a failure: the run skips whatever is already published. Worth saying out loud, though,
# because a fully published version means this release has nothing left to do.
if npm view "@knockdata/duckdb@$version" version >/dev/null 2>&1; then
	echo "note: @knockdata/duckdb@$version is already on npm; only missing packages will publish"
fi
